export default function OrderConfirmation({
  ...
  email,
  phone,
  address,       // e.g. "House 12, Street 5, DHA Phase 6"
  city,
  country,        // e.g. "Pakistan"
  orderId,        // optional — show if you have it
  paymentMethod,
  formatPrice,
  total,
  onContinueShopping,
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4 py-12">
      <div className="text-center max-w-md w-full">
        ...
        <div className="bg-white border border-[#333333] rounded-2xl p-5 text-left mb-6 space-y-4">
          {orderId && (
            <div>
              <p className="text-gray-500 text-sm mb-1">Order ID</p>
              <p className="text-[#333333] font-semibold break-all">{orderId}</p>
            </div>
          )}

          <div>
            <p className="text-gray-500 text-sm mb-1">Contact</p>
            <p className="text-[#333333] font-medium break-all">{email}</p>
            {phone && <p className="text-[#333333] font-medium">{phone}</p>}
          </div>

          {(address || city || country) && (
            <div>
              <p className="text-gray-500 text-sm mb-1">Delivery address</p>
              {address && <p className="text-[#333333] font-medium">{address}</p>}
          ...
          <div>
            <p className="text-gray-500 text-sm mb-1">Total charged</p>
